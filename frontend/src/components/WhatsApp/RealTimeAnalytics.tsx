            ...
            <div className="grid grid-cols-24 gap-1">
              {metrics.hourlyStats.map((stat, index) => (
                <div key={index} className="text-center">
                  <div
                    className="relative h-32 bg-gray-100 rounded-t"
                    style={{
                      background: `linear-gradient(to top, #3b82f6 0%, #3b82f6 ${(stat.sent / 60) * 100}%, #f3f4f6 ${(stat.sent / 60) * 100}%, #f3f4f6 100%)`
                    }}
                  >
                    <div className="absolute inset-x-0 top-0 text-xs text-gray-600 font-medium">
                      {stat.sent}
                    </div>
                  </div>
