...
export default function DiscountsReferralsPage() {
  ...
  const [referralForm, setReferralForm] = useState({
    ...
    referrer_discount_fixed: '25000',
    referrer_cash_amount: '30000',
    referred_installation_discount_fixed: '50000',
    referred_service_discount_fixed: '25000',
    ...
    try {
      ...
      if (activeTab === 'discounts') {
        ...
        const [discountsRes, statsRes] = await Promise.all([
          ...
          fetch('/api/v1/discounts/stats/summary')
        ]);
        ...
        if (statsRes.ok) {
          ...
          setDiscountStats(statsData.data);
        }
      } else {
  ...
  const loadTargetOptions = async (targetType: 'area' | 'package' | 'customer') => {
    try {
      ...
      if (targetType === 'area') {
        ...
        apiUrl = `/regions?${searchParams.toString()}`;
        ...
        searchParams.append('limit', '1000');
        apiUrl = `/packages?${searchParams.toString()}`;
  ...
  const handleDiscountSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingDiscount
        ? `/api/v1/discounts/${editingDiscount.id}`
        : '/api/v1/discounts';
      ...
      if (response.ok) {
        ...
        alert(error.error || 'Gagal menyimpan diskon');
      }
    } catch (error) {
  ...
  const handleDeleteDiscount = async (discountId: number) => {
    ...
    try {
      const response = await fetch(`/api/v1/discounts/${discountId}`, {
        method: 'DELETE',
      });
  ...
  const handleApplyDiscount = async (discountId: number) => {
    try {
      const response = await fetch(`/api/v1/discounts/${discountId}/apply`, {
        method: 'POST',
      });

      if (response.ok) {
        const result = await response.json();
        alert(`Diskon berhasil diterapkan ke ${result.data.appliedCount} tagihan`);
        fetchData();
      } else {
  ...
  const handleSaveReferral = async () => {
    try {
      const response = await fetch('/api/v1/referrals/settings', {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok && data.success) {
        ...
        alert(data.error || 'Gagal menyimpan pengaturan referral');
      }
    } catch (error) {
      console.error('Error saving referral settings:', error);
      alert('Terjadi kesalahan saat menyimpan pengaturan referral');
    }
  };
