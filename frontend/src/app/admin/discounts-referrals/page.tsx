...
export default function DiscountsReferralsPage() {
    ...
    try {
      ...
      if (activeTab === 'discounts') {
        ...
        const [discountsRes, statsRes] = await Promise.all([
          api.get('/discounts'),
          api.get('/discounts/stats/summary')
        ]);
  ...
  const handleDeleteDiscount = async (discountId: number) => {
    ...
    try {
      ...
      alert(error.response?.data?.error || 'Terjadi kesalahan saat menghapus diskon');
    }
  };
  ...
  const handleApplyDiscount = async (discountId: number) => {
    try {
      const response = await api.post(`/discounts/${discountId}/apply`);
  ...
  const handleSaveReferral = async () => {
    try {
      const response = await api.put('/referrals/settings', referralForm);
  ...
  const fetchBulkPaymentSettings = async () => {
    try {
      const response = await api.get('/billing/bulk-payment-settings');
  ...
  const handleSaveBulkPayment = async () => {
    try {
      const response = await api.put('/billing/bulk-payment-settings', bulkPaymentForm);
