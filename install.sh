...
if [[ "$DEPLOYMENT" == "docker"* ]]; then
    ...
    # Check if schema is loaded (by checking users table)
    print_info "Verifying database schema..."
    if ! $SUDO docker exec kilusi-postgres psql -U ${POSTGRES_USER:-kilusi_user} -d ${POSTGRES_DATABASE:-kilusi_bill} -c "SELECT count(*) FROM users;" > /dev/null 2>&1; then
        print_warning "Schema not found (users table missing). Loading schema manually..."
    ...
    $SUDO docker exec kilusi-postgres psql -U ${POSTGRES_USER:-kilusi_user} -d ${POSTGRES_DATABASE:-kilusi_bill} -c "
        ...
        ('${ADMIN_USERNAME:-admin}', '\$2b\$10\$X7V.7/8h.8/9.8/9.8/9.8/9.8/9.8/9', 'superadmin', 'admin@kilusi.id', NOW(), NOW())
...
if [[ "$DEPLOYMENT" == "native" ]] || [[ "$DEPLOYMENT" == "docker-db-radius" ]] || [[ "$DEPLOYMENT" == "docker-backend" ]]; then
    ...
    echo "# Generated by install.sh at $(date)" > .env.local
    if [ -n "$FRONTEND_API_URL" ]; then
            echo "NEXT_PUBLIC_API_URL=$FRONTEND_API_URL" >> .env.local
    else
            echo "# Leave empty to use Next.js Proxy (Single Host Mode)" >> .env.local
            echo "NEXT_PUBLIC_API_URL=" >> .env.local
    fi
    
    if [ -n "$PORTAL_URL" ]; then
        echo "NEXT_PUBLIC_PORTAL_URL=$PORTAL_URL" >> .env.local
    fi
    
    print_success "Frontend environment configured (.env.local generated)"
    print_info "API URL set to: '${FRONTEND_API_URL:-<Relative Proxy>}'"
