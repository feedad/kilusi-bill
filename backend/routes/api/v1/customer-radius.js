...
router.get('/info', verifyCustomerToken, async (req, res) => {
  try {
    ...
    console.log(`[DEBUG] Customer API: RADIUS connection for ${customer.pppoe_username}:`, radiusConnection);
    ...
    const radiusData = {
      ...
      session_end: null,
      session_duration: radiusConnection.online ? parseInt(radiusConnection.session_time) || 0 : 0,
    ...
    let realTimeSessionDuration = radiusData.session_duration || 0;
    if (radiusConnection.online && radiusData.session_start) {
      const sessionStart = new Date(radiusData.session_start);
      const now = new Date();
