...
router.get('/alerts', async (req, res) => {
    try {
        ...
        const onlineCustomersResult = await query(
            `SELECT COUNT(*) as online_count FROM customers WHERE status = 'online'`
        );
        ...
        for (const nas of nasResult.rows) {
            ...
            if (minutesSinceCheck > 10) {
                alerts.push({
                    ...
                    message: `NAS ${nas.nasname} has not responded for ${Math.floor(minutesSinceCheck)} minutes`,
    ...
    try {
        ...
        try {
            const recentNasUpdates = await query(
                `SELECT id, nasname, updated_at FROM nas_servers
            ...
            if (recentNasUpdates.rows && recentNasUpdates.rows.length > 0) {
                recentNasUpdates.rows.forEach(nas => {
                    activities.push({
                        ...
                        description: `NAS ${nas.nasname} status updated`,
                        user: 'System',
