...
router.get('/traffic', async (req, res) => {
    try {
        ...
        if (monitorMode === 'snmp') {
            ...
            // MikroTik mode
            const mikrotikTraffic = await getInterfaceTraffic(iface);
            traffic = {
                rx: mikrotikTraffic['rx-byte'] || 0,
                tx: mikrotikTraffic['tx-byte'] || 0,
                interface: iface,
                mode: 'mikrotik',
                timestamp: new Date().toISOString()
            };
...
router.get('/online-customers', async (req, res) => {
    try {
        ...
        try {
            ...
            `);
            onlineSessions = sessionsQuery.rows;
        } catch (err) {
...
router.get('/system-stats', async (req, res) => {
    try {
        ...
        if (monitorMode === 'snmp') {
            ...
            try {
                const resources = await getSystemResource();
                systemStats = {
                    ...systemStats,
