...
router.get('/stats', async (req, res) => {
    try {
        ...
        try {
            ...
            if (mikrotikAktif === 0) {
                logger.info('No sessions in customer_sessions table, trying RADIUS PostgreSQL fallback...');

                // Fallback 1: Try RADIUS PostgreSQL
                try {
                    const radiusDb = require('../../../config/radius-postgres');
                    const activeSessions = await radiusDb.getAllActiveSessions();
                    mikrotikAktif = activeSessions.length;
                    logger.info(`Found ${mikrotikAktif} online customers from RADIUS PostgreSQL`);
                } catch (radiusErr) {
                    logger.warn('RADIUS query failed, trying Mikrotik API:', radiusErr.message);

                    // Fallback 2: Try Mikrotik API
                    try {
                        const { getActivePPPoEConnections } = require('../../../config/mikrotik');
                        const pppoeResult = await getActivePPPoEConnections();
                        mikrotikAktif = pppoeResult.success ? pppoeResult.data.length : 0;
                        logger.info(`Found ${mikrotikAktif} online customers from Mikrotik API`);
                    } catch (mikrotikErr) {
                        logger.warn('Mikrotik API also failed:', mikrotikErr.message);
                        mikrotikAktif = 0; // Default to 0 if all methods fail
                    }
                }
            }
        } catch (err) {
