...
router.get('/', jwtAuth, async (req, res) => {
  try {
    ...
    let query = `
      ...
      LEFT JOIN customers c ON cr.customer_id = c.customer_id::text
      WHERE 1=1
    `;
    ...
    if (search) {
      query += ` AND (
        cr.customer_id ILIKE $${paramIndex} OR
    ...
    if (odp_id) {
      query += ` AND cr.odp_id = $${paramIndex}`;
      params.push(odp_id);
      paramIndex++;
    }
...
router.get('/stats', jwtAuth, async (req, res) => {
  try {
    const queries = await Promise.all([
      pool.query('SELECT COUNT(*) FROM cable_routes'),
      pool.query('SELECT COUNT(*) FROM cable_routes WHERE status = \'connected\''),
      pool.query('SELECT COUNT(*) FROM cable_routes WHERE status = \'disconnected\''),
      pool.query('SELECT COUNT(*) FROM cable_routes WHERE status = \'maintenance\''),
      pool.query('SELECT COUNT(*) FROM cable_routes WHERE status = \'damaged\''),
      pool.query('SELECT COUNT(DISTINCT odp_id) FROM cable_routes'),
      pool.query('SELECT COUNT(DISTINCT customer_id) FROM cable_routes'),
      pool.query('SELECT AVG(cable_length) FROM cable_routes WHERE cable_length IS NOT NULL')
    ]);
