...
router.get('/devices', async (req, res) => {
    try {
        ...
        logger.error('Error fetching GenieACS devices:', error);
        res.status(500).json({
            success: false,
            message: 'Terjadi kesalahan saat mengambil data perangkat GenieACS'
        });
    }
});
    ...
    try {
        ...
        try {
            ...
            res.json({
                success: true,
                data: result,
                message: `Device ${action} completed successfully`
            });
        } catch (err) {
            logger.error(`Failed to perform ${action} on device ${deviceId}:`, err);
            res.status(500).json({
                success: false,
                message: `Terjadi kesalahan saat melakukan aksi ${action} pada perangkat`
            });
        }
        ...
        res.status(500).json({
            success: false,
            message: 'Terjadi kesalahan saat melakukan aksi perangkat'
        });
    }
});
...
router.get('/stats', async (req, res) => {
    try {
        ...
        logger.error('Error fetching GenieACS stats:', error);
        res.status(500).json({
            success: false,
            message: 'Terjadi kesalahan saat mengambil statistik GenieACS'
        });
    }
});
