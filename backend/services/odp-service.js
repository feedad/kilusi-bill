    ...
    try {
        const stats = await getOne(`
            SELECT
                ...
                (SELECT COUNT(*) FROM cable_routes) as total_cable_routes,
                (SELECT COUNT(*) FROM cable_routes WHERE status = 'connected') as connected_routes,
                (SELECT COUNT(*) FROM cable_routes WHERE status = 'disconnected') as disconnected_routes,
                (SELECT COUNT(*) FROM cable_routes WHERE status = 'maintenance') as maintenance_routes,
                (SELECT SUM(capacity) FROM odps) as total_capacity,
                (SELECT COUNT(*) FROM customers WHERE odp_id IS NOT NULL) as total_connected_customers,
                (SELECT COUNT(*) FROM customers WHERE odp_id IS NOT NULL AND status = 'active') as total_active_customers,
