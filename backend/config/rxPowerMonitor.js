...
async function checkRXPowerAndNotify() {
  ...
  const warningThreshold = getSetting('rx_power_warning', -37);
  const criticalThreshold = getSetting('rx_power_critical', -40);
...
async function sendCriticalNotification(device, rxPowerValue, threshold) {
  ...
  const now = Date.now();
  const interval = getSetting('rx_power_notification_interval_minutes', 5) * 60 * 1000; // Convert menit ke ms
