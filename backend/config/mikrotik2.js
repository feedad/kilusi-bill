// Modul untuk koneksi dan operasi Mikrotik (optional)
...
if (useMikrotikApi) {
    try {
        ...
        logger.warn('node-routeros-v2 not installed; Mikrotik API disabled in mikrotik2 module.');
        RouterOSAPI = null;
    }
}
...
async function connectToMikrotik() {
    if (!useMikrotikApi || !RouterOSAPI) {
        logger.info('Mikrotik API is disabled (mikrotik2). Skipping connection.');
        return null;
    }
    try {
...
// Serialize writes and recover from unregistered tag errors
...
async function getInterfaceTraffic(interfaceName = 'ether1') {
    ...
    const res = await rosWrite('/interface/monitor-traffic', [
        ...
        logger.error('Error getting interface traffic:', error && (error.message || String(error)));
        return { rx: 0, tx: 0 };
    }
}
