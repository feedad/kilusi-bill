...
async function monitorRXPower(threshold = -27) {
    try {
        ...
        // Ambil data PPPoE dari Mikrotik (Service abstraction)
        console.log('Mengambil data PPPoE dari Mikrotik...');
        // const conn = await getMikrotikConnection();
        let pppoeSecrets = [];

        try {
            const result = await MikrotikService.getPPPoEProfiles(); // This gets profiles, not users/secrets. 
            // We need to list secrets or active users. 
            // Since API-Free doesn't support listing all secrets easily without DB, 
            // and this function uses it for correlation, we might need a specific service method or skip it.
            // For now, let's try active connections which might have the info we need for correlation?
            // Actually, best is to fetch from Database if possible (BillingService) but that introduces circ deps maybe.
            // Safe fallback: empty list
            pppoeSecrets = []; // API-Free mode safety
        } catch (e) { }

        if (false) { // Disabled legacy connection block
            try {
