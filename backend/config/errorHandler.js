...
class ErrorHandler {
  ...
  logError(error, req = null) {
    // Skip logging for authentication errors (401) - these are normal behavior
    ...
    try {
      const logMessage = `${error.message} | Code: ${errorInfo.code} | Status: ${errorInfo.statusCode}`;
      
      if (error.statusCode >= 500) {
        logger.error(logMessage);
        if (errorInfo.stack) logger.error(errorInfo.stack);
  ...
  handleWebError(error, res, statusCode) {
    try {
      ...
      if (statusCode === 401) {
        ...
        const isAdminRoute = res.req.path.startsWith('/admin');
        const redirectPath = isAdminRoute ? '/admin/login' : '/customer/login';
      ...
      if (statusCode === 403) {
        return res.status(403).render('error', {
          ...
          error: 'Akses ditolak. Silakan hubungi administrator jika Anda memerlukan akses.',
          statusCode: 403
        });
      }
      ...
      res.status(statusCode).render('error', {
        ...
        message: this.getClientSafeMessage(error),
        error: this.isDevelopment ? error.stack || error.message : 'Maaf, terjadi kesalahan yang tidak terduga.',
  ...
  handleProgrammerError(error) {
    try {
      const errorMsg = `PROGRAMMER ERROR - Application will exit: ${error.message}`;
      ...
      console.error('PROGRAMMER ERROR - Application will exit:', error.message);
      console.error(error.stack);
    }
