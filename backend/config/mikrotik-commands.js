...
async function handleAddHotspotUser(remoteJid, params) {
    ...
    const result = { success: false, message: "Not supported in API-Free mode" }; // await MikrotikService.addHotspotUser(username, password, profile);
...
async function handleAddPPPoESecret(remoteJid, params) {
    ...
    const result = { success: false, message: "Not supported in API-Free mode" }; // await MikrotikService.addPPPoESecret(username, password, profile, localAddress);
...
async function handleResourceInfo(remoteJid) {
    ...
    const result = await MikrotikService.getSystemResources(); // Using getSystemResources which maps to getResourceInfo equivalent
...
async function handleActiveHotspotUsers(remoteJid) {
    ...
    const result = { success: true, data: [] }; // await MikrotikService.getActiveHotspotUsers();
    if (!result.success) {
        await sock.sendMessage(remoteJid, { text: `❌ ${result.message}` });
        return;
    }
...
async function handleActivePPPoE(remoteJid) {
    ...
    const result = await MikrotikService.getActivePPPoEConnections();
    if (!result.success) {
        await sock.sendMessage(remoteJid, { text: `❌ ${result.message}` });
        return;
    }
...
async function handleDeleteHotspotUser(remoteJid, params) {
    ...
    const result = { success: false, message: "Not supported in API-Free mode" }; // await MikrotikService.deleteHotspotUser(username);
...
async function handleDeletePPPoESecret(remoteJid, params) {
    ...
    const [username] = params;
    const result = { success: false, message: "Not supported in API-Free mode" }; // await MikrotikService.deletePPPoESecret(username);
...
async function handleOfflineUsers(remoteJid) {
    ...
    const result = { success: true, totalSecrets: 0, totalActive: 0, totalInactive: 0, data: [] }; // await MikrotikService.getInactivePPPoEUsers();
...
async function handleInterfaceDetail(remoteJid, params) {
    ...
    const result = await MikrotikService.getInterfaceDetail(interfaceName);
...
async function handleInterfaceStatus(remoteJid, params, enable) {
    ...
    const [interfaceName] = params;
    const result = await MikrotikService.setInterfaceStatus(interfaceName, enable);
...
async function handleIPAddresses(remoteJid) {
    ...
    const result = await MikrotikService.getIPAddresses();
    if (!result.success) {
        await sock.sendMessage(remoteJid, { text: `❌ ${result.message}` });
        return;
    }
...
async function handleRoutes(remoteJid) {
    ...
    const result = await MikrotikService.getRoutes();
    if (!result.success) {
        await sock.sendMessage(remoteJid, { text: `❌ ${result.message}` });
        return;
    }
...
async function handleDHCPLeases(remoteJid) {
    ...
    const result = await MikrotikService.getDHCPLeases();
    if (!result.success) {
        await sock.sendMessage(remoteJid, { text: `❌ ${result.message}` });
        return;
    }
...
async function handlePing(remoteJid, params) {
    ...
    const result = { success: false, message: "Not supported in API-Free mode" }; // await MikrotikService.pingHost(host, count);
...
async function handleSystemLogs(remoteJid, params) {
    ...
    const result = { success: false, message: "Not supported in API-Free mode" }; // await MikrotikService.getSystemLogs(topics, count);
...
async function handleProfiles(remoteJid, params) {
    ...
    if (type === 'pppoe' || type === 'all') {
        const pppoeResult = await MikrotikService.getPPPoEProfiles();
    ...
    if (type === 'hotspot' || type === 'all') {
        const hotspotResult = { success: true, data: [] }; // await MikrotikService.getHotspotProfiles();
...
async function handleFirewall(remoteJid, params) {
    ...
    const result = { success: false, message: "Not supported in API-Free mode" }; // await MikrotikService.getFirewallRules(chain);
    if (!result.success) {
        await sock.sendMessage(remoteJid, { text: `❌ ${result.message}` });
        return;
    }
...
async function handleConfirmRestart(remoteJid) {
    ...
    const result = { success: false, message: "Not supported in API-Free mode" }; // await MikrotikService.restartRouter();
...
async function handleRouterIdentity(remoteJid, params) {
    ...
    if (params.length === 0) {
        ...
        const result = await MikrotikService.getRouterIdentity();
        if (!result.success) {
            await sock.sendMessage(remoteJid, { text: `❌ ${result.message}` });
            return;
        }
        ...
        const result = await MikrotikService.setRouterIdentity(newName);
...
async function handleRouterClock(remoteJid) {
    ...
    const result = await MikrotikService.getRouterClock();
    if (!result.success) {
        await sock.sendMessage(remoteJid, { text: `❌ ${result.message}` });
        return;
    }
...
async function handleAllUsers(remoteJid) {
    ...
    const result = { success: true, data: { totalActive: 0, totalOffline: 0, hotspotActive: [], pppoeActive: [], pppoeOffline: [] } }; // await MikrotikService.getAllUsers();
    if (!result.success) {
        await sock.sendMessage(remoteJid, { text: `❌ ${result.message}` });
        return;
    }
...
async function handleDebugResource(remoteJid) {
    ...
    try {
        const rawData = await MikrotikService.getSystemResources(); // Using resources as proxy for debug
