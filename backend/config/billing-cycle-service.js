...
class BillingCycleService {
     ...
     */
    async getCustomerBillingCycle(customerId) {
        try {
            ...
            if (customer && customer.siklus) {
                return customer.siklus;
            }
    ...
    async updateCustomerBillingDates(customerId) {
        try {
            // Get customer data
            const customer = await getOne(`
                SELECT c.active_date, p.profile_period
                FROM customers c
                LEFT JOIN packages p ON c.package_id = p.id
                WHERE c.id = $1
            `, [customerId]);

            if (!customer || !customer.active_date) {
                throw new Error('Customer or active date not found');
            }
            ...
            logger.error('Error updating customer billing dates:', error);
            throw error;
        }
    }
    ...
    async recalculateAllCustomerBillingDates() {
        try {
            ...
            const customers = await query(`
                SELECT c.id, c.active_date, p.profile_period
                FROM customers c
                LEFT JOIN packages p ON c.package_id = p.id
