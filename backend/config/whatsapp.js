...
// Get super admin number from environment variable
function getSuperAdminNumber() {
    const superAdmin = process.env.WHATSAPP_SUPER_ADMIN;
    if (!superAdmin) {
        console.warn('⚠️ WHATSAPP_SUPER_ADMIN not set! Super admin features disabled.');
        return null;
    }
    return superAdmin.trim();
}
                    ...
                    try {
                        ...
                        */
                        ...
                        try {
                            ...
                            const superAdminNumber = process.env.WHATSAPP_SUPER_ADMIN || null;
                            ...
                            if (superAdminNumber && superAdminNumber !== adminNumber) {
                                ...
                                console.log('⚠️ Super admin tidak dikonfigurasi (WHATSAPP_SUPER_ADMIN env not set)');
                            }
                        } catch (error) {
...
async function handleIncomingMessage(sock, message) {
    ...
    if (!global.superAdminWelcomeSent) {
        try {
            const superAdminNumber = process.env.WHATSAPP_SUPER_ADMIN || process.env.ADMIN_NUMBER;
