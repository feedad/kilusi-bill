...
async function getActivePPPoEConnections() {
    try {
        // Use MikrotikService instead of direct connection
        return await MikrotikService.getActivePPPoEConnections();
    } catch (error) {
...
async function getOfflinePPPoEUsers(activeUsers) {
    try {
        // Since we are in API-Free mode, listing "offline" users implies knowing ALL users.
        // If using RADIUS, we'd query the database.
        // If using Mikrotik API (legacy), we'd query secrets.
        // For now, we stub this or use MikrotikService if extended.
        // But since MikrotikService is currently basic, we might return empty list or "Not Supported".
        // HOWEVER, MikrotikService *could* implement getInactivePPPoEUsers.
        // Let's defer to a safe empty list for now to avoid crash.
        return [];
