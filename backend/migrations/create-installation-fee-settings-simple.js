...
async function createInstallationFeeSettings() {
  try {
    console.log('🔄 Creating simple installation_fee_settings table...');
    ...
    if (!tableExists) {
      // Create simple installation_fee_settings table
      await query(`
    ...
    for (const setting of defaultSettings) {
      await query(`
        INSERT INTO installation_fee_settings (billing_type, fee_amount, description)
        VALUES ($1, $2, $3)
    ...
    settingsResult.rows.forEach((setting, index) => {
      console.log(`${index + 1}. ${setting.billing_type.toUpperCase()}`);
